...
List concatList(List A, List B) {
	if (A == NULL || B == NULL) {
                fprintf(stderr, "Error: calling concatList() on NULL List object(s).\n");
                exit(EXIT_FAILURE);
        }
	...
	while (i < A->length && A->front != NULL) {
		append(outList, A->front->data);
		i++;
	}

	while (i < B->length && B->front != NULL) {
		append(outList, B->front->data);
		i++;
	}
